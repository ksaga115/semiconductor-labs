...
**入口は [`index.html`](index.html)（学びの地図）。** ブラウザで開くと、第0〜9部・付録と9ラボの目次と、章ごとに対応する
ラボの課題へのリンクが出る。この README はその Markdown 版（要点だけ）。

半導体を、足場（第0部）から、製造工程 → 素子の物理 → 論理回路 → イメージセンサ → 光検出デバイス → アナログ回路 → 光学 → 信頼性と品質 → 光源とレーザー（第9部）までの10部と、物性値と索引の付録で通して学ぶための場所。
各部は **参考書**（読んで理解する）と **ラボ**（自分の手で作って、機械に採点させる）の二層になっている（第0部・第9部・付録は参考書のみ）。
...
半導体\
  ...
  第5部_光検出デバイス.html          第5部 参考書 ― 数える一族: PMT・APD・MPPC・SNSPD から EMMI・光る半導体まで（8章）
  第6部_アナログ回路.html           第6部 参考書 ― 坂に住む設計: gmとroから差動対・帰還・雑音・バンドギャップ、AD 変換・電源・実装と EMC まで（11章）
  第7部_光学.html                 第7部 参考書 ― 検出器の相手側: 照度の会計・結像・回折・ガウス・ファイバ・膜、偏光・干渉・分光器・損失と分散（12章）
  第8部_信頼性と品質.html           第8部 参考書 ― 壊れ方を数える: FIT・アレニウス・ワイブル・θとTj・TEC・Cpk・誤差、管理図・不確かさ・故障解析・規格と安全（12章）
  第9部_光源とレーザー.html          第9部 参考書 ― 光をつくる側: 熱放射・放電・LED・レーザーの原理とレート方程式・縦モード・DFB/VCSEL・パルス・非線形（11章）
  付録_物性値と索引.html            付録 ― 物性値の表・式の索引・全部の部の用語の統一索引（検索できる）
...
| 目的 | 順 |
|---|---|
| はじめて（体系の順に全部通す） | 第0部 → 第1部 参考書 → ProcessLab 1–3章 → SemiLab 1–3章 → ProcessLab 4章 → SemiLab 4–5章・CharLab → 第3部・NandLab → 第6部・AnalogLab → 第7部・OptoLab → 第9部 → 第5部・PhotonLab → 第4部・PixelLab → 第8部・QALab → 付録 |
