...
- 各ラボの「ここから先」（README）― BJT・降伏・反射防止膜の設計・pMOS（SemiLab。ただし dev.js の MOS 解は p 基板＋n+poly ゲート前提で、n 基板に対しては正しい Vth を返さないことを確認済み ― pMOS 課題はソルバの n 基板対応が先）、側壁と形状・TED（ProcessLab）、光学（PixelLab。HDR 合成は「長短合成で 100 dB」として課題化済み）、メモリ（NandLab。RAM16 を「実装部品」として追加済み ― 課題の採点では使えない）、多段OTA（AnalogLab。SCは課題化済み）、モード結合・TCSPCパイルアップ（OptoLab/PhotonLab。走行時間・エテンデュ・cos⁴・背景光・計数は課題化済み）、Norris-Landzberg・HAST・GR&R の分散分析（QALab。Peck と Coffin-Manson は課題化済み）。さらに先の候補: デバイス特性ラボ（C-V/I-V の実測練習）
