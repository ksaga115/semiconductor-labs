 ...
 * まったく違う層の並びでも通る。
 *
 * ar は反射防止スライダの値、coat は反射防止膜 { mat, n, dnm }（どちらも構造の外にある摘み）。
 */
(function (global) {
  'use strict';
  ...
  var A = {
    ...
    quarter: {
      coat: { mat: 'sin', n: 2.0, dnm: 75 },
      note: '「光が見える」の 3 層に Si₃N₄ を 75 nm（600/(4×2.00)）。600 nm の反射 35.5% → 0.005%、量子効率 61% → 95%。',
      make: function () {
        return build([['si', 300, { na: 1e19 }], ['si', 10000, { nd: 1e15 }], ['si', 2000, { nd: 1e19 }]]);
      }
    },
    index: {
      coat: { mat: 'custom', n: 1.99, dnm: 75.4 },
      note: 'n = 1.99（≈ √3.95）・厚み 75.4 nm（600/(4×1.99)）。600 nm の反射はほぼ 0。',
      make: function () {
        return build([['si', 300, { na: 1e19 }], ['si', 10000, { nd: 1e15 }], ['si', 2000, { nd: 1e19 }]]);
      }
    },
