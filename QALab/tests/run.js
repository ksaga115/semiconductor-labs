...
T('故障率の上限・保護めがね・アラン偏差 ― 第8部 01・11〜13 の数字', () => {
  near('m(0 個・60%) = −ln 0.4 = 0.916', M.poisUpper(0, 0.6), -Math.log(0.4), 1e-9);
  near('m(0 個・90%) = ln 10 = 2.303', M.poisUpper(0, 0.9), Math.log(10), 1e-9);
  within('m(1 個・60%) = χ²(0.6; 4)/2 ≒ 2.022', M.poisUpper(1, 0.6), 2.0223, 1.0005);
  within('m(1 個・90%) = χ²(0.9; 4)/2 ≒ 3.890', M.poisUpper(1, 0.9), 3.8897, 1.0005);
  within('231 個・1,000 h・0 故障で 51 FIT（第8部 01 Q2）', evalWith({ nfa: 231, rfa: 0 }).ev.fitUp, 51.09, 1.002);
  within('0 故障で 10 FIT に要る個数 ≒ 1,180（第8部 01 の例題）', M.poisUpper(0, 0.6) / (1e-8 * 1000 * evalWith({}).ev.af), 1180, 1.001);
  within('1 個の故障を見込むと、231 個では 113 FIT', evalWith({ nfa: 231, rfa: 1 }).ev.fitUp, 112.8, 1.002);
  near('100 mW を OD 2 で 1 mW（第8部 12 の例題）', evalWith({ lpw: 100, od: 2 }).ev.odT, 1, 1e-9);
  near('必要な OD = log₁₀(100/1) = 2', evalWith({}).ev.odNeed, 2, 1e-12);
  near('OD 3 は 1/1,000', evalWith({ lpw: 1, od: 3 }).ev.odT, 0.001, 1e-12);
  near('最適な平均の時間 (1/0.001)^(2/3) = 100 s（第8部 13 の図）', evalWith({}).ev.tOpt, 100, 1e-9);
  within('そのときの σ = 0.1225', evalWith({}).ev.sigMin, 0.12247, 1.0005);
  ok('長く平均すると悪くなる（1,000 s で 0.708）', evalWith({ tavg: 1000 }).ev.sigA > 0.7);
});
