...
const OWN = ['char', 'quest', 'answer', 'store', 'ui', 'main'];
...
T('起動と答案', () => {
  ...
  ok('答案の欄がある', $(doc, 'designForm').querySelectorAll('input').length >= 9);
  ok('測定表が出る', /ダイオード/.test($(doc, 'derived').textContent));
  ok('描けている', $(doc, 'board').getContext()._stack === 0);
  ok('ステータスに進み', /当てたパラメータ/.test($(doc, 'statRight').textContent));

  const inputs = $(doc, 'designForm').querySelectorAll('input');
  inputs[0].value = '1.36'; inputs[0].fire('change'); win.flush();
  eq('n を数値で書けた', S.design.nfit, 1.36);
  ...
  eq('読めない値では変わらない', S.design.nfit, 1.36);
  const in2 = $(doc, 'designForm').querySelectorAll('input');
  in2[5].value = '9999'; in2[5].fire('change'); win.flush();
  eq('範囲の外は端に丸める（S 値 ≤ 500）', S.design.ssfit, 500);
});

T('課題の一巡', () => {
  const { doc, S, win, CL } = boot({ search: '?fresh=1' });
  CL.quest.LIST.forEach((q) => {
    CL.ui.selectQuest(q.id); win.flush();
  ...
  eq('全部 ✓', Object.keys(S.cleared).length, CL.quest.LIST.length);

  /* わざと外すと落ちる */
  CL.ui.selectQuest('dn'); win.flush();
  S.design.nfit = 2.0;
  $(doc, 'btnGrade').fire('click'); win.flush();
  ok('外れた読みは落ちる', /まだ/.test($(doc, 'qResult').textContent));
});

T('保存', () => {
  const { win, S } = boot({ seed: '{壊れ' });
  ok('壊れていても起動する', !!S.design);
  ok('壊れた中身は退避', win.localStorage.getItem('CharLab.broken') !== null);
  const g = boot({ seed: JSON.stringify({ v: 1, design: { nfit: 1.36 } }) });
  eq('保存から復元（n）', g.S.design.nfit, 1.36);
  ok('足りない項目は既定で埋まる', typeof g.S.design.toxfit === 'number');
});
