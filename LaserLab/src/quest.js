...
(function (global) {
  ...
  var CH = [
    ...
    { id: 5, name: '第5章　光源を道具として使う', lead: '輝線で目盛りを合わせる・量子欠損の熱・輝度でファイバに入れる。第9部 02・08・11。' }
  ];
  ...
  var LIST = [
    ...
    /* ===== 第5章 ===== */
    {
      id: 'calib', ch: 5, kind: 'design',
      name: '輝線 2 本で目盛りを合わせる',
      desc: '350〜800 nm の分光器の波長の目盛りを、**水銀の輝線 2 本**で直線に合わせる。1 本の線の読みのばらつき **σ = 0.02 nm**（固定）のまま、'
          + '範囲の外の **700 nm** での目盛りの誤差を **0.035 nm 以下**にする。使える線は 365.02・404.66・435.83・546.07・576.96・579.07 nm（253.65 nm は範囲の外）。',
      why: '2 点で決めた直線の誤差は、2 本の線の間では小さく、外に出るほど膨らむ: σ·√((λ − λ<sub>b</sub>)² + (λ − λ<sub>a</sub>)²)/|λ<sub>b</sub> − λ<sub>a</sub>|（第9部 02）。'
         + '1 本だけではずれの傾きが分からず、近い 2 本では傾きの誤差が大きい ― <b>範囲の両端に近い線で合わせる</b>のが作法。'
         + '576.96 と 579.07 nm は 2.1 nm しか離れていない二重線なので、分解能がそれより粗い分光器では 1 本に重なる（ここでは分けて読めると仮定）。',
      hint: '435.83 と 546.07 nm では 0.056 nm。いちばん離れた 365.02 と 579.07 nm で 0.033 nm。404.66 と 579.07 nm は 0.037 nm で届かない。',
      check: function (ev, d) {
        var lock = near(d.caltgt, 700) && near(d.calsig, 0.02);
        var inA = ev.calLineA !== null && ev.calLineA >= 350 && ev.calLineA <= 800;
        var inB = ev.calLineB !== null && ev.calLineB >= 350 && ev.calLineB <= 800;
        var two = inA && inB && Math.abs(d.calA - d.calB) > 0.1;
        var okE = two && ev.calErr <= 0.035;
        return {
          ok: lock && okE,
          rows: [
            row('700 nm の目盛りの誤差', f(ev.calErr, 4) + ' nm', '0.0350 以下', okE),
            row('使った線', f(d.calA, 3) + ' / ' + f(d.calB, 3) + ' nm', '水銀の輝線（350〜800 nm）の 2 本', two),
            row('条件固定', lock ? '守っている（700 nm・σ 0.02 nm）' : '課題の条件に戻す', '', lock)
          ]
        };
      }
    },
    {
      id: 'defect', ch: 5, kind: 'design',
      name: 'Yb ファイバで 100 W・熱 10 W 以下',
      desc: 'Yb 添加ファイバのレーザーで **100 W** を取り出す。励起は Yb の吸収帯（**915 ± 5 nm** または **976 ± 2 nm**）の LD、発振の波長は **1,064〜1,080 nm**（用途の都合、仮定）。'
          + '量子欠損だけで出る熱を **10 W 以下**にする。',
      why: '量子欠損 1 − λ<sub>p</sub>/λ<sub>s</sub> は、励起の光子のエネルギーのうち熱になる割合（第9部 08）。出力 P を取り出すと、欠損だけで P(λ<sub>s</sub>/λ<sub>p</sub> − 1) の熱が出る。'
         + '915 nm の吸収帯は広くて励起の波長の管理が楽だが欠損が大きく、976 nm の吸収帯は狭い（LD の波長を温度などで合わせる）が欠損が小さい ― <b>熱と波長の管理の取り引き</b>。',
      hint: '976 nm 励起なら発振 1,073.6 nm まで 10 W 以下（1,070 nm で 9.63 W）。915 nm 励起では 1,006.5 nm より長いと必ず 10 W を超える。',
      check: function (ev, d) {
        var lock = near(d.pout, 100);
        var band = (d.pumpnm >= 910 && d.pumpnm <= 920) || (d.pumpnm >= 974 && d.pumpnm <= 978);
        var okS = d.signm >= 1064 && d.signm <= 1080;
        var okH = ev.heatW <= 10;
        return {
          ok: lock && band && okS && okH,
          rows: [
            row('量子欠損の熱', f(ev.heatW, 2) + ' W（欠損 ' + f(ev.qd * 100, 2) + ' %・励起 ' + f(ev.pumpW, 1) + ' W）', '10.00 以下', okH),
            row('励起の波長', f(d.pumpnm, 1) + ' nm', 'Yb の吸収帯 915 ± 5 / 976 ± 2 nm', band),
            row('発振の波長', f(d.signm, 1) + ' nm', '1,064〜1,080 nm', okS),
            row('条件固定', lock ? '守っている（100 W）' : '課題の条件に戻す', '', lock)
          ]
        };
      }
    },
    {
      id: 'bright', ch: 5, kind: 'design',
      name: 'LED の光をファイバに 1 mW',
      desc: '発光面 **1 mm²**・出力 **1 W** の LED（ランバート面、輝度 3.18×10⁵ W/(m²·sr)）の光を、レンズを使ってマルチモードのファイバに **1.0 mW 以上**入れる。'
          + 'ファイバは石英の標準品の範囲で、コア径 **200 µm まで**・NA **0.22 まで**。',
      why: 'レンズで像を縮めると角度が広がって NA からあふれ、像を広げるとコアからあふれる。入る光の上限は <b>輝度 × ファイバのエテンデュ（コアの面積 × πNA²）</b>で、'
         + 'レンズでは輝度を上げられない（第9部 11・第7部 06）。上限を上げるには、ファイバの側（コアと NA）を大きくするか、輝度の高い光源（SLD・レーザー）に替えるしかない。',
      hint: 'NA 0.22 ならコア 162 µm 以上、コア 200 µm なら NA 0.178 以上。100 µm・0.22 では 0.38 mW。シングルモード（エテンデュ ≈ λ²）なら 1,550 nm で 0.77 µW。',
      check: function (ev, d) {
        var lock = near(d.ledP, 1) && near(d.ledA, 1);
        var lim = d.fcore <= 200 && d.fna <= 0.22;
        var okP = ev.pFibMw >= 1.0;
        return {
          ok: lock && lim && okP,
          rows: [
            row('ファイバに入る光', f(ev.pFibMw, 3) + ' mW', '1.000 以上', okP),
            row('ファイバ', f(d.fcore, 0) + ' µm・NA ' + f(d.fna, 3), 'コア 200 µm まで・NA 0.22 まで', lim),
            row('輝度 × エテンデュ', ev.radiance.toExponential(2) + ' W/(m²·sr) × ' + ev.etFib.toExponential(2) + ' m²·sr', '', true),
            row('条件固定', lock ? '守っている（1 W・1 mm²）' : '課題の条件に戻す', '', lock)
          ]
        };
      }
    }
