...
const OWN = ['photon', 'quest', 'answer', 'store', 'ui', 'main'];
...
T('起動と設計', () => {
  ...
  ok('設計の欄がある', $(doc, 'designForm').querySelectorAll('input').length >= 14);
  ok('設計から出る数字がある', /NEP/.test($(doc, 'derived').textContent));
  ok('描けている', $(doc, 'board').getContext()._stack === 0);
  ok('ステータスに SNR', /SNR/.test($(doc, 'statLeft').textContent));

  const inputs = $(doc, 'designForm').querySelectorAll('input');
  inputs[0].value = '850'; inputs[0].fire('change'); win.flush();
  eq('波長を数値で変えた', S.design.nm, 850);
  ...
  eq('読めない値では変わらない', S.design.nm, 850);
  const in2 = $(doc, 'designForm').querySelectorAll('input');
  in2[3].value = '9999'; in2[3].fire('change'); win.flush();
  eq('範囲の外は端に丸める（M ≤ 300）', S.design.M, 300);
});

T('課題の一巡', () => {
  const { doc, S, win, PH } = boot({ search: '?fresh=1' });
  PH.quest.LIST.forEach((q) => {
    PH.ui.selectQuest(q.id); win.flush();
  ...
  eq('全部 ✓', Object.keys(S.cleared).length, PH.quest.LIST.length);

  /* わざと崩すと落ちる */
  PH.ui.selectQuest('fast'); win.flush();
  S.design.cpf = 2;
  $(doc, 'btnGrade').fire('click'); win.flush();
...
T('保存', () => {
  ...
  ok('壊れた中身は退避', win.localStorage.getItem('PhotonLab.broken') !== null);
  const g = boot({ seed: JSON.stringify({ v: 1, design: { nm: 905 } }) });
  eq('保存から復元（波長）', g.S.design.nm, 905);
  ok('足りない項目は既定で埋まる', typeof g.S.design.cpf === 'number');
});
