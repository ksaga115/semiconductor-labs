...
(function (global) {
  ...
  var CH = [
    ...
    { id: 5, name: '第5章　エネルギー・速さ・直線性', lead: 'X 線の分解能（ファノと ENC）、不感時間の数え落とし、フォトダイオードの直線性。第5部 09・12・14。' }
  ];
  ...
  var LIST = [
    ...
    /* ===== 第5章 ===== */
    {
      id: 'xres', ch: 5, kind: 'design',
      name: 'SDD で 5.9 keV を 130 eV に',
      desc: '条件固定: **5.9 keV**（⁵⁵Fe の Mn Kα）を**シリコン**（w **3.62 eV**・ファノ因子 **0.115**）で直接数える。回路の雑音 ENC だけを動かして、エネルギー分解能を **130 eV FWHM 以下**にする。ただしこの回路で出せる ENC は **4 e⁻ まで**（仮定）。',
      why: 'FWHM = 2.355·w·√(F·N + ENC²)（第5部 09）。対の数 N = 5,900/3.62 = 1,630 の揺らぎはファノ因子で縮み、それだけで 117 eV が床になる。'
         + '回路の雑音は二乗和で乗るので、ENC 5 e⁻ で 124 eV、10 e⁻ で 145 eV ― **低いエネルギーほど、分解能は回路が決める**。SDD が陽極を小さくして容量を減らしたのは、この ENC を下げるため。',
      hint: '130 eV には √(F·N + ENC²) ≤ 15.25、つまり ENC ≤ 6.7 e⁻。窓は 4〜6.7 e⁻。',
      check: function (ev, d) {
        var lock = near(d.ekev, 5.9) && near(d.wev, 3.62) && near(d.fano, 0.115);
        var okR = ev.xFwhm <= 130, lim = d.enc >= 4;
        return {
          ok: lock && okR && lim,
          rows: [
            row('分解能（FWHM）', f(ev.xFwhm, 1) + ' eV（' + f(ev.xRel * 100, 2) + ' %・ファノの限界 ' + f(ev.xFano, 1) + ' eV）', '130.0 以下', okR),
            row('対の数 N', f(ev.xN, 0), '', true),
            row('ENC', f(d.enc, 2) + ' e⁻', '4 以上（回路の下限）', lim),
            row('条件固定', lock ? '守っている（5.9 keV・w 3.62 eV・F 0.115）' : '課題の条件に戻す', '', lock)
          ]
        };
      }
    },
    {
      id: 'dead', ch: 5, kind: 'design',
      name: '数え落としを 5% に収める',
      desc: '条件固定: 真の計数率 **2×10⁶ /s**、計数回路は**非拡張型**（型の欄 0）。不感時間 τ で、数え落としを **5% 以下**にする。ただしパルスの幅のため τ は **10 ns より短くできない**（仮定）。',
      why: '非拡張型では m = n/(1 + nτ)、数え落としは nτ/(1 + nτ)（第5部 12）。τ を縮めれば数え落としは減るが、パルスの幅より短い不感時間は作れない ― 弁別器とパルスの整形が下限を決める。'
         + '数え落としが 10% を超えるほど強い光は、式で補正するより、減光するか電流の読みに替えるのが作法。',
      hint: 'nτ/(1 + nτ) ≤ 0.05 から τ ≤ 26.3 ns。窓は 10〜26.3 ns。20 ns で 3.85%。',
      check: function (ev, d) {
        var lock = near(d.ntrue, 2e6) && d.dtype === 0;
        var okL = ev.deadLoss <= 0.05, lim = d.dtau >= 10;
        return {
          ok: lock && okL && lim,
          rows: [
            row('数え落とし', f(ev.deadLoss * 100, 2) + ' %（数えた率 ' + ex(ev.mCount, 3) + ' /s）', '5.00 以下', okL),
            row('不感時間 τ', f(d.dtau, 1) + ' ns', '10 以上（パルスの幅）', lim),
            row('条件固定', lock ? '守っている（2×10⁶ /s・非拡張型）' : '課題の条件に戻す', '', lock)
          ]
        };
      }
    },
    {
      id: 'pdlin', ch: 5, kind: 'design',
      name: 'PD で 100 µA を 1% の直線性で',
      desc: '条件固定: 第5部 14 の仮定の PD（I<sub>s</sub> 25.9 pA・R<sub>s</sub> 10 Ω・C<sub>j</sub> 15 pF・V<sub>bi</sub> 0.6 V）を **10 kΩ** の負荷で受け、光電流 **100 µA** を目減り **1% 以内**で読む。逆バイアスで、あわせて帯域を **1.5 MHz 以上**にする。逆バイアスは素子の定格で **5 V まで**（仮定）。',
      why: '負荷で受けると接合が I·R<sub>L</sub> ぶん順方向に振られ、光電流がダイオードへ逃げる（第5部 14）。零バイアスの 1% の上限は 23.8 µA で足りない。'
         + '逆バイアスをかけると順方向に振られるまでの余裕が増え、しかも空乏層が広がって C<sub>j</sub> が減り速くなる ― **直線性のための逆バイアスに、速さがついてくる**。',
      hint: '100 µA を 1% で通すには約 0.72 V。そのとき C_j 10.1 pF・帯域 1.57 MHz。窓は約 0.72〜5 V。',
      check: function (ev, d) {
        var lock = near(d.pdua, 100) && near(d.rlk, 10);
        var okR = ev.pdRatio >= 0.99, okB = ev.pdBw >= 1.5e6, lim = d.vr >= 0 && d.vr <= 5;
        return {
          ok: lock && okR && okB && lim,
          rows: [
            row('負荷に届く割合', f(ev.pdRatio * 100, 2) + ' %（接合 ' + f(ev.pdVj, 3) + ' V）', '99.00 以上', okR),
            row('帯域（10 kΩ・C_j ' + f(ev.pdCj * 1e12, 2) + ' pF）', isFinite(ev.pdBw) ? f(ev.pdBw / 1e6, 2) + ' MHz' : '―', '1.50 以上', okB),
            row('逆バイアス', f(d.vr, 2) + ' V', '5 V まで', lim),
            row('条件固定', lock ? '守っている（100 µA・10 kΩ）' : '課題の条件に戻す（TIA に替えるのは別の答え）', '', lock)
          ]
        };
      }
    }
