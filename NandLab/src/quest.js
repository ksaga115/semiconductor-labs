...
(function (global) {
  ...
  var QUESTS = [
    ...
    {
      id: 'parity4', name: '4ビットパリティ', goal: 12, stage: 2, needs: ['xor'],
      desc: '1 の個数が奇数なら 1。4入力の XOR。',
      why: '1ビットどこが化けても、答えが必ず反転する ― 誤りの「検出」ができる。メモリや通信が化けに気づける最小の仕掛けで、次のハミング符号の部品そのもの。',
      hint: 'XOR は「2入力のパリティ」。XOR(XOR(A0,A1), XOR(A2,A3)) と木に組む。',
      kind: 'comb', inputs: names('A', 4), outputs: ['Y'],
      rows: build(4, function (b) { return [b[0] ^ b[1] ^ b[2] ^ b[3]]; })
    },
    {
      id: 'ham74', name: 'ハミング符号器', goal: 24, stage: 2, needs: ['parity4'],
      desc: 'データ D1..D4 から検査ビットを作る。P1 = D1⊕D2⊕D4、P2 = D1⊕D3⊕D4、P3 = D2⊕D3⊕D4。',
      why: '3つの検査は、7つの位置を「どの検査が見るか」で塗り分けたパリティ。1ビット化けると、合わない検査の組がそのまま化けた位置の2進番号になる ― 検出から訂正へ進む鍵。第3部「メモリの物理」で触れた ECC の実体がこれ。',
      hint: '3入力のパリティを3回。見る部分集合が違うだけで、どれも XOR 2個で組める。',
      kind: 'comb', inputs: ['D1', 'D2', 'D3', 'D4'], outputs: ['P1', 'P2', 'P3'],
      rows: build(4, function (b) {
        return [b[0] ^ b[1] ^ b[3], b[0] ^ b[2] ^ b[3], b[1] ^ b[2] ^ b[3]];
      })
    },
    {
      id: 'hamfix', name: 'ハミング訂正器', goal: 71, stage: 2, needs: ['ham74', 'dec24'],
      desc: '受け取った7ビット R1..R7（データは R3,R5,R6,R7）から、1ビットまでの化けを直したデータ D1..D4 を出す。',
      why: '検査 S1=R1⊕R3⊕R5⊕R7、S2=R2⊕R3⊕R6⊕R7、S4=R4⊕R5⊕R6⊕R7 を取り直すと、(S4S2S1) が化けた位置の2進番号になる。位置が分かれば XOR で反転して直すだけ ― 「気づく」から「直す」へ。ただし2ビット化けると別の場所を"直して"しまう（距離3の限界。実用のメモリ ECC が検出用のパリティをもう1本足す理由）。',
      hint: 'シンドローム3本（4入力パリティ）→ 位置の一致検出（デコーダの発想: NOT と AND の木）→ 該当するデータだけ XOR で反転。',
      kind: 'comb', inputs: ['R1', 'R2', 'R3', 'R4', 'R5', 'R6', 'R7'], outputs: ['D1', 'D2', 'D3', 'D4'],
      rows: build(7, function (b) {
        var s1 = b[0] ^ b[2] ^ b[4] ^ b[6];
        var s2 = b[1] ^ b[2] ^ b[5] ^ b[6];
        var s4 = b[3] ^ b[4] ^ b[5] ^ b[6];
        var pos = s4 * 4 + s2 * 2 + s1;
        var d = [b[2], b[4], b[5], b[6]];
        if (pos === 3) d[0] ^= 1;
        if (pos === 5) d[1] ^= 1;
        if (pos === 6) d[2] ^= 1;
        if (pos === 7) d[3] ^= 1;
