...
(function (global) {
  ...
  var CH = [
    ...
    { id: 5, name: '第5章　像の質と膜と計測', lead: 'MTF と焦点深度・多層膜の鏡・計測カメラの被写界深度（第7部 15・16・18）。' }
  ];
  ...
  var LIST = [
    ...
    /* ===== 第5章 ===== */
    {
      id: 'mtf', ch: 5, kind: 'design',
      name: 'MTF と焦点深度の窓 ― F 値を選ぶ',
      desc: '画素 **3.45 µm**・**550 nm**・無限遠のまま、F 値で、ナイキスト（145 lp/mm）での系の MTF（回折 × 画素の開口）を **0.30 以上**にし、像の側の焦点深度 2Nc（c = 2 画素 = 6.9 µm）を **60 µm 以上**にする。',
      why: '絞るほど焦点深度は 2Nc で深くなるが、回折の遮断周波数 1/(λN) が下がって MTF が落ちる（第7部 04・15）。F2.8 の系はナイキストで 0.457、F8 では 0.157 ― <b>窓は両側から閉じる</b>。'
         + '組み立ての誤差やセンサの傾きを吸収するのが焦点深度で、それを細かさと取り合う。',
      hint: 'MTF ≥ 0.30 は N ≤ 5.38、焦点深度 ≥ 60 µm は N ≥ 4.35。F4.8 で MTF 0.334・焦点深度 66 µm。',
      check: function (ev, d) {
        var lock = near(d.nm, 550) && near(d.ppum, 3.45);
        var okM = ev.mtfSys >= 0.30, okF = ev.focusUm >= 60;
        return {
          ok: lock && okM && okF,
          rows: [
            row('ナイキストでの系の MTF', f(ev.mtfSys, 3) + '（回折 ' + f(ev.mtfLens, 3) + ' × 画素 ' + f(ev.mtfPix, 3) + '）', '0.300 以上', okM),
            row('焦点深度 2Nc', f(ev.focusUm, 1) + ' µm（F' + f(d.N, 2) + '・c ' + f(ev.cUm, 1) + ' µm）', '60.0 以上', okF),
            row('条件固定', lock ? '守っている（550 nm・画素 3.45 µm）' : '課題の条件に戻す', '', lock)
          ]
        };
      }
    },
    {
      id: 'hr', ch: 5, kind: 'design',
      name: 'ダイクロイックの鏡 ― 500〜620 nm を 99.5% で返す',
      desc: 'TiO₂（**2.35**）と SiO₂（**1.46**）をガラス（**1.52**）の上に λ/4 で重ねたまま、対の数 N と中心の波長で、中心の反射を **99.5% 以上**にし、高反射帯で **500〜620 nm** を覆う。層の数（2N+1）は **15 まで**（膜の応力と成膜の時間の予算、仮定）。',
      why: '対を足すほど中心の反射は 100% に近づくが、帯の幅は屈折率の比だけで決まり、層を足しても広がらない（第7部 16）。'
         + 'だから覆いたい帯には中心の波長で合わせ、反射の高さは対の数で稼ぐ ― <b>2 つの摘みが別々の仕事をする</b>。',
      hint: 'N = 5 で 99.06%、N = 6 で 99.64%。帯の幅 Δg = 0.300 なので、中心 λ₀ は 620 × 0.85 = 527 〜 500 × 1.15 = 575 nm。',
      check: function (ev, d) {
        var lock = near(d.nH, 2.35) && near(d.nL, 1.46) && near(d.nsg, 1.52);
        var okR = ev.Rml >= 0.995, okB = ev.bandLo <= 500 && ev.bandHi >= 620, lim = ev.layers <= 15;
        return {
          ok: lock && okR && okB && lim,
          rows: [
            row('中心の反射', f(ev.Rml * 100, 3) + ' %', '99.500 以上', okR),
            row('高反射帯', f(ev.bandLo, 1) + '〜' + f(ev.bandHi, 1) + ' nm（中心 ' + f(d.lam0, 1) + ' nm）', '500〜620 nm を覆う', okB),
            row('層の数', f(ev.layers, 0) + ' 層（対 ' + f(d.npair, 0) + '）', '15 以下', lim),
            row('条件固定', lock ? '守っている（TiO₂ 2.35・SiO₂ 1.46・ガラス 1.52）' : '課題の条件に戻す', '', lock)
          ]
        };
      }
    },
    {
      id: 'dof', ch: 5, kind: 'design',
      name: '計測カメラの被写界深度 ― 深さと回折の窓',
      desc: '**f 50 mm・物体距離 500 mm**（m = 0.111）・画素 **3.45 µm**・**550 nm** のまま、F 値で、物体の側の被写界深度 2Nc(1+m)/m²（c = 2 画素）を **5 mm 以上**にし、回折の点像 2.44λN(1+m) を c 以下に抑える。',
      why: '絞れば深くなるが、点像が太ってボケの許容 c を超える（第7部 04・18）。<b>深さの上限は、画素の大きさと回折で決まっている</b> ―'
         + 'この系では F4.6 で 5.7 mm が天井で、それ以上深くしたければ画素を大きくするか倍率を下げる。',
      hint: '深さ ≥ 5 mm は N ≥ 4.03、回折 ≤ 6.9 µm は N ≤ 4.63。F4.4 で深さ 5.47 mm・点像 6.56 µm。',
      check: function (ev, d) {
        var lock = near(d.fmm, 50) && near(d.amm, 500) && near(d.ppum, 3.45) && near(d.nm, 550);
        var okD = ev.dofMm >= 5, okA = ev.diffUm <= ev.cUm;
        return {
          ok: lock && okD && okA,
          rows: [
            row('被写界深度', f(ev.dofMm, 2) + ' mm（F' + f(d.N, 2) + '・m ' + f(ev.mag, 3) + '）', '5.00 以上', okD),
            row('回折の点像', f(ev.diffUm, 2) + ' µm', f(ev.cUm, 2) + '（2 画素）以下', okA),
            row('条件固定', lock ? '守っている（f 50・a 500・画素 3.45 µm・550 nm）' : '課題の条件に戻す', '', lock)
          ]
        };
      }
    }
