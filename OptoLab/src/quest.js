...
(function (global) {
  ...
  var LIST = [
    ...
    },
    {
      ...
      why: '軸外の照度は主光線角 θ の cos⁴ で落ちる ― 距離が cos で伸び、面を斜めに見込む cos が2回、瞳が楕円に見える cos が1回。'
         + '**tanθ = 像高/f なので、広角ほど隅が暗い**。標準 50mm でフルサイズの隅は 71%、広角 35mm では 53%。'
         + '第4部の現像が最初にやる「レンズシェーディング補正」の物理側の根拠で、補正はゲインで持ち上げる ― つまり**隅は SNR でも損**をしている。',
      hint: 'f ≥ 49 mm で 70% に乗る。50mm（θ=23.4°）が定番の答え。35mm は 53% で落ちる。',
      check: function (ev, d) {
