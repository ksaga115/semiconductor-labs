...
const OWN = ['analog', 'quest', 'answer', 'store', 'ui', 'main'];
...
T('起動と設計', () => {
  ...
  ok('設計の欄がある', $(doc, 'designForm').querySelectorAll('input').length >= 10);
  ok('設計から出る数字がある', /GBW/.test($(doc, 'derived').textContent));
  ok('描けている', $(doc, 'board').getContext()._stack === 0);
  ok('ステータスに gm', /gm/.test($(doc, 'statLeft').textContent));

  const inputs = $(doc, 'designForm').querySelectorAll('input');
  inputs[2].value = '25'; inputs[2].fire('change'); win.flush();
  eq('W/L を数値で変えた', S.design.wl, 25);
  const bad = $(doc, 'designForm').querySelectorAll('input')[2];
  bad.value = 'abc'; bad.fire('change'); win.flush();
  ok('読めない値は赤くなる', bad.classList.contains('bad'));
  eq('読めない値では変わらない', S.design.wl, 25);
  const in2 = $(doc, 'designForm').querySelectorAll('input');
  in2[3].value = '99999'; in2[3].fire('change'); win.flush();
  eq('範囲の外は端に丸める（Id ≤ 2000µA）', S.design.idua, 2000);
});

T('課題の一巡', () => {
  const { doc, S, win, AN } = boot({ search: '?fresh=1' });
  AN.quest.LIST.forEach((q) => {
    AN.ui.selectQuest(q.id); win.flush();
  ...
  eq('全部 ✓', Object.keys(S.cleared).length, AN.quest.LIST.length);

  /* わざと崩すと落ちる */
  AN.ui.selectQuest('tia'); win.flush();
  S.design.rfk = 100;
  $(doc, 'btnGrade').fire('click'); win.flush();
...
T('保存', () => {
  ...
  ok('壊れた中身は退避', win.localStorage.getItem('AnalogLab.broken') !== null);
  const g = boot({ seed: JSON.stringify({ v: 1, design: { wl: 42 } }) });
  eq('保存から復元（W/L）', g.S.design.wl, 42);
  ok('足りない項目は既定で埋まる', typeof g.S.design.rdk === 'number');
});
...
T('ランチャと README', () => {
  ...
  if (fs.existsSync(vbs)) {
    ...
    ok('日本語が壊れていない', /アナログラボ/.test(text));
  }
