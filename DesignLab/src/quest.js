...
(function (global) {
  ...
  var CH = [
    ...
    { id: 4, name: '第4章　PET と蛍光寿命', lead: 'エネルギーの窓・同時計数と TOF・TCSPC の繰り返しとパイルアップ。第10部 10・11。' }
  ];
  var PET = { egam: 511, ly: 30, lcol: 0.3, pdep: 0.3, pct: 0.05, rint: 8 };
  ...
  var LIST = [
    ...
    },
    {
      ...
      desc: '**10 秒**の露光で、1 画素の暗電流を読み出し雑音の二乗（**2.56 e⁻**）以下にする。暗電流は冷やす前 **10 e⁻/画素/s**（仮定）で、'
          + '**約 9 ℃ ごとに 2 倍**（目安。300 K 付近の生成電流で実際に 2 倍になる幅は 8.6 K）。TEC は Q<sub>max</sub> **10 W**・ΔT<sub>max</sub> **70 K** で、熱負荷 **1.5 W** を吸えなければならない。',
    ...
    /* ===== 第4章 PET と蛍光寿命 ===== */
    {
      id: 'petwin', ch: 4, kind: 'design',
      name: 'エネルギーの窓で散乱を落とす',
      desc: 'LYSO **30 光子/keV**・集光 **0.3**・検出効率 **0.3**・クロストーク **5%**・固有の分解能 **8%**（合わせて 10.3%）のまま、'
          + 'エネルギーの窓の下限で、**30° より大きく曲がった散乱をすべて落とし**、本物の 511 keV を **99% 以上**残す。',
      why: '体の中でコンプトン散乱して θ 曲がった 511 keV の光子は E′ = 511/(2 − cos θ) keV まで下がる（第10部 10）。'
         + '窓の下限を上げるほど大きく曲がった散乱を落とせるが、本物の光電ピークも分解能（σ ≈ 22 keV）の裾から削れる ― '
         + '<b>窓の幅は分解能で決まる</b>。分解能が悪いと窓を広げざるをえず、散乱を多く拾って像のコントラストが下がる。',
      hint: '30° の散乱は 511/(2 − cos 30°) = 450.6 keV。99% を残す下限は 511 − 2.33σ = 459.0 keV。窓は 450.6〜459.0 keV。',
      check: function (ev, d) {
        var p = ev.pe, lock = locks(d, PET);
        var okT = isFinite(p.thetaCut) && p.thetaCut <= 30, okK = p.keep >= 0.99;
        return {
          ok: lock && okT && okK,
          rows: [
            row('落とせる散乱', 'θ > ' + f(p.thetaCut, 1) + '°（窓の下限 ' + f(d.lwin, 1) + ' keV）', '30.0° 以下から落とす', okT),
            row('本物を残す割合', f(p.keep * 100, 2) + ' %（分解能 ' + f(p.resPct, 1) + ' %・σ ' + f(p.sigmaKeV, 1) + ' keV）', '99.00 以上', okK),
            lockRow(lock, '30 光子/keV・集光 0.3・PDE 0.3・クロストーク 5%・固有 8%')
          ]
        };
      }
    },
    {
      id: 'tof', ch: 4, kind: 'design',
      name: '同時計数の窓と TOF',
      desc: '直径 **70 cm** の視野・両側の単独の計数 **1×10⁵ /s**（固定）のまま、同時計数の窓 W を視野を光が横切る時間以上にとって本物の組を取り逃さず、'
          + '偶発同時計数を **30 /s 以下**に抑え、時刻の分解能で TOF の位置の精度 c·Δt/2 を **2.5 cm 以下**にする。'
          + '時刻の分解能は **100 ps より良くはできない**（仮定）。',
      why: '偶発同時計数は R = W·S₁·S₂ で、窓を広げるほど増える（第10部 10）。だが窓は、視野の端どうしから来た本物の組の到着の差（直径/c）より狭くできない ― '
         + '<b>窓は両側から閉じる</b>。TOF は到着の差から線の上の位置を c·Δt/2 で決め、200 ps で 3.0 cm。時刻が細かいほど、同じ数の光子で像の雑音が減る。',
      hint: '70 cm / c = 2.33 ns 以上、30 / (10⁵)² = 3.0 ns 以下。2.5 cm には時刻の分解能 166.8 ps 以下。',
      check: function (ev, d) {
        var p = ev.pe, lock = locks(d, { fov: 70, sing: 1e5 });
        var okW = d.wcoin >= p.wminNs - 1e-12, okR = p.randoms <= 30 + 1e-9, okT = p.tofCm <= 2.5, lim = d.ctr >= 100;
        return {
          ok: lock && okW && okR && okT && lim,
          rows: [
            row('同時計数の窓', f(d.wcoin, 2) + ' ns（視野を横切る時間 ' + f(p.wminNs, 2) + ' ns）', '横切る時間以上', okW),
            row('偶発同時計数', f(p.randoms, 1) + ' /s', '30.0 以下', okR),
            row('TOF の位置の精度', f(p.tofCm, 2) + ' cm（時刻の分解能 ' + f(d.ctr, 0) + ' ps）', '2.50 以下', okT),
            row('時刻の分解能の下限', f(d.ctr, 0) + ' ps', '100 以上（仮定）', lim),
            lockRow(lock, '視野 70 cm・単独の計数 1×10⁵ /s')
          ]
        };
      }
    },
    {
      id: 'flrep', ch: 4, kind: 'design',
      name: '寿命 10 ns の繰り返しを選ぶ',
      desc: '寿命 **10 ns** の試料を、1 パルスで **0.01 個**・1 画素 **10⁴ 個**（寿命の精度 1%）・**256 × 256 画素**のまま、'
          + '繰り返しで前のパルスからの持ち越しを **0.1% 以下**にし、画像 1 枚を **90 分以内**に撮る。',
      why: '持ち越しの比 e^(−T/τ)/(1 − e^(−T/τ)) は、周期 T を寿命の何倍にとるかで決まる（第10部 11）。だが繰り返しを下げると、'
         + '1 パルス 0.01 個の約束のもとで計数も下がり、時間が延びる ― <b>繰り返しは速さではなく寿命で決め、時間は検出器を並べて稼ぐ</b>。',
      hint: '0.1% には T ≥ τ·ln 1001 = 69.1 ns → 14.47 MHz 以下。90 分には計数 1.21×10⁵ /s → 12.2 MHz 以上。',
      check: function (ev, d) {
        var q = ev.fl, lock = locks(d, { tauf: 10, mu: 0.01, nphf: 1e4, npxl: 256 });
        var okC = q.carry <= 0.001, okT = q.timgS <= 5400;
        return {
          ok: lock && okC && okT,
          rows: [
            row('持ち越し', f(q.carry * 100, 4) + ' %（周期 ' + f(q.Tns, 1) + ' ns ＝ ' + f(q.Tns / d.tauf, 2) + 'τ）', '0.1000 以下', okC),
            row('画像 1 枚', f(q.timgS / 60, 1) + ' 分（計数 ' + e(q.rate, 2) + ' /s・1 画素 ' + f(q.tpixMs, 1) + ' ms）', '90.0 以下', okT),
            lockRow(lock, '寿命 10 ns・0.01 個・10⁴ 個・256²')
          ]
        };
      }
    },
    {
      id: 'flmu', ch: 4, kind: 'design',
      name: 'パイルアップと撮る時間',
      desc: '寿命 **2.5 ns**・**40 MHz**・1 画素 **10⁴ 個**・**256 × 256 画素**のまま、1 パルスあたりの平均の検出数 µ で、'
          + '2 個以上だった検出の割合（パイルアップ）を **2% 以下**にし、画像 1 枚を **15 分以内**に撮る。',
      why: 'TCSPC は 1 周期に最初の 1 個しか記録しないので、2 個以上来たパルスでは早く来た光子に偏り、寿命が短く見える。その割合は約 µ/2（第10部 11・第5部 12）。'
         + 'µ を上げれば速く撮れるが偏る ― <b>数える率は繰り返しの数 % までが目安</b>。',
      hint: '2% は µ ≈ 0.040 まで。15 分には計数 7.28×10⁵ /s、1 − e^(−µ) ≥ 0.0182 から µ ≥ 0.0184。',
      check: function (ev, d) {
        var q = ev.fl, lock = locks(d, { tauf: 2.5, fflim: 40, nphf: 1e4, npxl: 256 });
        var okP = q.pile <= 0.02, okT = q.timgS <= 900;
        return {
          ok: lock && okP && okT,
          rows: [
            row('パイルアップ', f(q.pile * 100, 2) + ' %（µ ' + f(d.mu, 4) + '）', '2.00 以下', okP),
            row('画像 1 枚', f(q.timgS / 60, 2) + ' 分（計数 ' + e(q.rate, 2) + ' /s）', '15.00 以下', okT),
            lockRow(lock, '寿命 2.5 ns・40 MHz・10⁴ 個・256²')
          ]
        };
      }
    }
